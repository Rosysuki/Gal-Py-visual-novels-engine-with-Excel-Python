...
class GalPy(object):
    ...
    ifdefine: Any = re_compile(r"\$\s?[Ii][Ff][Dd][Ee][Ff][Ii][Nn][Ee](.*)")
    ifndefine: Any = re_compile(r"\$\s?[Ii][Ff][Nn][Dd][Ee][Ff][Ii][Nn][Ee](.*)")
    enddefine: Any = re_compile(r"\$\s?([Ee][Nn][Dd][Dd][Ee][Ff][Ii][Nn][Ee])")
...
class Parser(object):
    ...
    def macro(each: Any ,
        ...
        if (res := GalPy.define.findall(str(each))).__len__():
            print(f"define {res}")
            __dict[res[0][0]]: dict[str : str] = res[0][1].strip()
            return
        ...
        if (res := GalPy.include_all.findall(str(each))).__len__():
            ...
            for i ,e in enumerate(GalPy.ALLMODULE):
                Parser.__get_included(e ,__dict ,__register)
            return

        if (res := GalPy.include.findall(str(each))).__len__():
            #__include.add(eval(res[0]))
            Parser.__get_included(eval(res[0]) ,__dict ,__register)
            return
    ...
    def __count_passed(self ,start: int) -> int:
        """
        only call in macro!
        """
        for i ,e in enumerate(self.__list[start+1:] ,start=0):
            if GalPy.enddefine.findall(str(e)).__len__():
                return i + start
        else:
            stderr.write(f"Error In Line{start}:\n缺失enddefine!\n")
            return -1
    ...
    def __lshift__(self ,name: str) -> bool:
        ...
        ENDDEFINE: dict[str : Any] = {"gate" : False ,"key" : 0}
        for index ,each in enumerate(self.__list[:] ,start=0):

            if '$' not in each.__str__():
                continue

            #print(f"{__dict=}")

            if ENDDEFINE["gate"]:
                if index.__eq__(ENDDEFINE["key"]):
                    ENDDEFINE["gate"] ,ENDDEFINE["key"] = False ,0
                continue

            #print(each)

            if (res := GalPy.ifdefine.findall(str(each))).__len__():
                if (res_status := self.__count_passed(index)).__eq__(-1):
                    return
                
                if (res := res[0].strip()) not in __dict:
                    ENDDEFINE["gate"] ,ENDDEFINE["key"] = True ,res_status
                    stderr.write("CAN'T FOUND MACRO-defined {}!\n".format(res))
                    continue
                pass

            if (res := GalPy.ifndefine.findall(str(each))).__len__():
                if (res_status := self.__count_passed(index)).__eq__(-1):
                    return

                #print(f"{__dict=}")
                if (res := res[0].strip()) in __dict:
                    ENDDEFINE["gate"] ,ENDDEFINE["key"] = True ,res_status
                    stderr.write("MACRO {} IS DEFINED!\n".format(res))
                    continue
                pass
            
            Parser.macro(each ,__dict ,__include ,__register)
        ...
        #for index ,each in enumerate(__include ,start=1):
            #print(f"{each=}")
        #    Parser.__get_included(each ,__dict ,__register)
        ...
        for index ,each in enumerate(self.__list ,start=1):
            ...
            if each and "$" not in each:
                ...
                for j ,e in enumerate(__register ,start=1):
                    if (res := re_findall(r"(.*){}(.*)".format(e) ,each.strip())).__len__():
                        if each.strip().count(e) == 1:
                            each: str = ''.join([' ' * GalPy.count_space(each) ,res[0][0] ,e ,"(" ,res[0][1].strip() ,")"])
                        else:
                            showerror("错误" ,"过于复杂!")
                            stderr.write("语句有歧义！\n")
                            return
                ...
                while (res := re_findall(r"(.*)[Ll][Ee][Tt]\s+(\S+)\s+->\s(.+)\s*" ,each.strip())).__len__():
                    each: str = ' ' * GalPy.count_space(each) + ''.join([res[0][0] ,res[0][1] ,'=' ,res[0][2]])
